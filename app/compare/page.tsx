...
export default function ComparePage() {
  ...
  useEffect(() => {
    async function fetchCars() {
      if (carIds.length === 0) { setCars([]); setLoading(false); return }
      setLoading(true)
      try {
        const fetched: ComparisonCar[] = []
        for (const id of carIds) {
          const res = await fetch(`/api/cars/${id}`)
          if (res.ok) { const data = await res.json(); if (data) fetched.push(data) }
        }
        setCars(fetched)
      } catch (err) { console.error('Failed to fetch comparison cars:', err) }
  ...
  const lowestPurchaseMonthly = useMemo(() => {
    let lowest = Infinity
    for (const r of results) { for (const o of r.origins) { const m = r.purchase[o]?.monthlyEquivalent; if (m != null && m < lowest) lowest = m } }
    return lowest
  }, [results])

  const lowestFlexleaseMonthly = useMemo(() => {
    let lowest = Infinity
    for (const r of results) { if (r.flexlease.monthlyEquivalent < lowest) lowest = r.flexlease.monthlyEquivalent }
    return lowest
  }, [results])
  ...
  const handleClearAll = useCallback(() => { clearCompare(); setCarIds([]) }, [])

  const handleOpenAddModal = useCallback(async () => {
    setShowAddModal(true); setSearchQuery('')
    if (allCars.length === 0) {
      try { const res = await fetch('/api/cars?limit=200'); const data = await res.json(); setAllCars(data.cars ?? []) }
      catch { setAllCars([]) }
    }
  }, [allCars.length])
  ...
  const handleAddCar = useCallback((carId: string) => {
    ...
    if (ids.includes(carId) || ids.length >= 5) return
    ids.push(carId)
    localStorage.setItem('carscout-compare-cars', JSON.stringify(ids))
    setCarIds([...ids]); setShowAddModal(false)
  }, [])

  const filteredAllCars = useMemo(() => {
    if (!searchQuery.trim()) return allCars
    const q = searchQuery.toLowerCase()
    return allCars.filter((c: ComparisonCar) => `${c.brand} ${c.model} ${c.variant}`.toLowerCase().includes(q))
  }, [allCars, searchQuery])

  function fmt(num: number): string { return new Intl.NumberFormat('da-DK').format(Math.round(num)) }
    ...
    <main className="min-h-screen bg-background">
      <header className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <Link href="/" className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors">
                ...
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary"><Search className="h-4 w-4 text-primary-foreground" /></div>
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-muted-foreground">{results.length} / 5 biler</span>
              <button onClick={handleOpenAddModal} disabled={carIds.length >= 5} className="flex items-center gap-1 rounded-lg border border-border bg-secondary px-3 py-1.5 text-sm text-foreground hover:bg-secondary/80 transition-colors disabled:opacity-50">
                <Plus className="h-3.5 w-3.5" /><span className="hidden sm:inline">Tilf&#248;j bil</span>
              </button>
              {carIds.length > 0 && (
                <button onClick={handleClearAll} className="flex items-center gap-1 rounded-lg border border-border bg-secondary px-3 py-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
                  <Trash2 className="h-3.5 w-3.5" /><span className="hidden sm:inline">Ryd</span>
                </button>
              )}
              <button onClick={() => setShowSettings(!showSettings)} className={`rounded-lg border border-border p-1.5 transition-colors ${showSettings ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground hover:text-foreground'}`}>
        ...
        <div className="flex-1 overflow-x-auto p-4 sm:p-6">
          {loading ? (
            <div className="flex items-center justify-center h-64"><div className="text-muted-foreground">Indl&#230;ser biler...</div></div>
          ) : results.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-center">
              <p className="text-lg text-muted-foreground mb-4">Ingen biler tilf&#248;jet til sammenligning</p>
              <button onClick={handleOpenAddModal} className="flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors">
                <Plus className="h-4 w-4" />Tilf&#248;j din f&#248;rste bil
              </button>
            </div>
          ) : (
            <div className="flex gap-4 pb-4">
              ...
              {carIds.length < 5 && results.length > 0 && (
                <button onClick={handleOpenAddModal} className="min-w-[320px] border-2 border-dashed border-border rounded-xl flex flex-col items-center justify-center gap-2 text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-colors h-64">
                  <Plus className="h-8 w-8" /><span className="text-sm">Tilf&#248;j bil</span>
                </button>
              )}
            </div>
          )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50" onClick={() => setShowAddModal(false)}>
          <div className="w-full max-w-lg max-h-[80vh] rounded-xl border border-border bg-card shadow-xl flex flex-col" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
            <div className="p-4 border-b border-border">
              ...
              <input type="text" placeholder="S&#248;g m&#230;rke, model..." value={searchQuery} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearchQuery(e.target.value)} className="w-full rounded-md border border-border bg-secondary px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground" autoFocus />
            </div>
            <div className="overflow-y-auto flex-1 p-2">
              {filteredAllCars.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-8">Ingen biler fundet</p>
              ) : (
                filteredAllCars.map((car: ComparisonCar) => {
                  const alreadyAdded = carIds.includes(car.id)
                  return (
                    <button key={car.id} onClick={() => !alreadyAdded && handleAddCar(car.id)} disabled={alreadyAdded} className={`w-full flex items-center gap-3 rounded-lg p-2 text-left transition-colors ${alreadyAdded ? 'opacity-50 cursor-not-allowed' : 'hover:bg-secondary cursor-pointer'}`}>
                      {car.stored_image_url ? (
                        <img src={car.stored_image_url} alt={`${car.brand} ${car.model}`} className="h-12 w-20 rounded object-cover shrink-0" />
                      ) : (
                        <div className="flex h-12 w-20 items-center justify-center rounded bg-secondary text-sm font-bold text-muted-foreground shrink-0">{car.brand?.charAt(0) ?? '?'}</div>
                      )}
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{car.first_registration_year} &middot; {fmt(car.mileage_km)} km &middot; {car.price_amount} {car.price_currency}</p>
                      </div>
                      {alreadyAdded && (<span className="text-xs text-muted-foreground ml-auto shrink-0">Tilf&#248;jet</span>)}
                    </button>
                  )
                })
              )}
